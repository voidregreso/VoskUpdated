...
class SpeechStreamService(
    ...
    fun start(listener: RecognitionListener?): Boolean {
        ...
        recognizerThread = listener?.let { RecognizerThread(it) }
    ...
    fun start(listener: RecognitionListener?, timeout: Int): Boolean {
        if (null != recognizerThread) return false
        recognizerThread = listener?.let { RecognizerThread(it, timeout) }
        recognizerThread!!.start()
        return true
    }
    ...
    private inner class RecognizerThread @JvmOverloads constructor(
        ...
        override fun run() {
            ...
            while (!interrupted() && (timeoutSamples == Companion.NO_TIMEOUT || remainingSamples > 0)) {
                try {
                    val nread = inputStream.read(buffer, 0, buffer.size)
                    if (nread < 0) {
                        break
                    } else {
                        val isSilence = recognizer.acceptWaveForm(buffer, nread)
                        if (isSilence) {
